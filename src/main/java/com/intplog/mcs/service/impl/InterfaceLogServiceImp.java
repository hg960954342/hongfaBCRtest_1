...
public class InterfaceLogServiceImp implements InterfaceLogService {
        ...
        @Override
        public int insertInterfaceLog(String method, Object rq, Date rqTime, jsonDataGcs rp, Date rpTime) {
                InterfaceLog log = new InterfaceLog();
                String id = UUID.randomUUID().toString().replaceAll("-", "");
                Integer status = 0; //0为正常，1为错误
        ...
        @Override
        public int insertInterFaceLogByResPCross(String method, Object rq, Date rqTime, jsonDataGcs rp, Date rpTime) {
                InterfaceLog log = new InterfaceLog();
                String id = UUID.randomUUID().toString().replaceAll("-", "");
                Integer status = 0; //0为正常，1为错误
